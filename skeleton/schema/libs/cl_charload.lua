...
local gradient_u = surface.GetTextureID("vgui/gradient-u")
...
function PANEL:Paint(width, height)
    ...
    cam.Start3D(self.cameraPosition, self.cameraAngle, modelFOV, x, y, width, height)
        ...
        for i = 1, 4 do
            render.SetModelLighting(i, 0.3, 0.3, 0.3)
        end

        render.SetModelLighting(5, 0.04, 0.04, 0.4)
...
function PANEL:Init()
    ...
    local back = controlList:Add("RMenuSelectionButton")
    back:SetText(L("return"))
    back:Dock(BOTTOM)
    back:DockMargin(0, 10, 0, 0)
    back:SetBackgroundColor(ix.config.Get("color"))
    local tW, tH = back:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    back:SetTall(tH)
    back:SetWide(tW)
    ...
    local continueButton = infoButtons:Add("RMenuSelectionButton")
    continueButton:Dock(LEFT)
    continueButton:SetText("choose")
    local tW, tH = continueButton:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    continueButton:SetTall(tH)
    continueButton:SetWide(tW)
    ...
    local deleteButton = infoButtons:Add("RMenuSelectionButton")
    deleteButton:Dock(LEFT)
    deleteButton:SetText("delete")
    local tW, tH = deleteButton:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    deleteButton:SetTall(tH)
    deleteButton:SetWide(tW)
    ...
    local deleteReturn = deleteInfo:Add("RMenuSelectionButton")
    deleteReturn:Dock(BOTTOM)
    deleteReturn:SetText("no")
    local tW, tH = deleteReturn:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    deleteReturn:SetTall(tH)
    deleteReturn:SetWide(tW)

    deleteReturn.DoClick = function()
        self:SetActiveSubpanel("main")
    end

    local deleteConfirm = self.delete:Add("RMenuSelectionButton")
    deleteConfirm:Dock(BOTTOM)
    deleteConfirm:SetText("yes")
    local tW, tH = deleteConfirm:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    deleteConfirm:SetTall(tH)
    deleteConfirm:SetWide(tW)
...
function PANEL:Populate(ignoreID)
    ...
    for i = 1, #ix.characters do
        ...
        local button = self.characterList:Add("RMenuSelectionButton")
