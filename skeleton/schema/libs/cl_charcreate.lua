...
function PANEL:Init()
	...
	modelList.Paint = function(self, width, height) derma.SkinFunc("PaintCharacterCreateBackground", self, width, height) end

	local proceed = modelList:Add("RMenuButton")
	proceed:SetText("proceed")
	proceed:Dock(BOTTOM)

    local tW, tH = proceed:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    proceed:SetTall(tH)
    proceed:SetWide(tW)
	...
	self.factionModel.PaintModel = function(self, width, height) derma.SkinFunc("PaintCharacterCreateBackground", self, width, height) end
	...
	local factionBack = self.factionPanel:Add("RMenuButton")
	factionBack:SetText("return")
	factionBack:Dock(BOTTOM)
    
    local tW, tH = factionBack:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    factionBack:SetTall(tH)
    factionBack:SetWide(tW)
	...
	local descriptionBack = descriptionModelList:Add("RMenuButton")
	descriptionBack:SetText("return")
	descriptionBack:Dock(BOTTOM)

    local tW, tH = descriptionBack:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    descriptionBack:SetTall(tH)
    descriptionBack:SetWide(tW)
	...
	local descriptionProceed = self.descriptionPanel:Add("RMenuButton")
	descriptionProceed:SetText("proceed")
	descriptionProceed:Dock(BOTTOM)

    local tW, tH = descriptionProceed:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    descriptionProceed:SetTall(tH)
    descriptionProceed:SetWide(tW)
	...
	local attributesBack = attributesModelList:Add("RMenuButton")
	attributesBack:SetText("return")
	attributesBack:Dock(BOTTOM)

    local tW, tH = attributesBack:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    attributesBack:SetTall(tH)
    attributesBack:SetWide(tW)
	...
	local create = self.attributesPanel:Add("RMenuButton")
	create:SetText("finish")
	create:Dock(BOTTOM)

    local tW, tH = create:GetTextSize()
    tW = tW + 24
    tH = tH + 32
    create:SetTall(tH)
    create:SetWide(tW)
...
function PANEL:Populate()
	if (!self.bInitialPopulate) then
		...
		for _, v in SortedPairs(ix.faction.teams) do
			if (ix.faction.HasWhitelist(v.index)) then
				local button = self.factionButtonsPanel:Add("RMenuSelectionButton")
				button:SetBackgroundColor(v.color or color_white)
				button:SetText(L(v.name):utf8upper())

                local tW, tH = button:GetTextSize()
                tW = tW + 24
                tH = tH + 32
                button:SetTall(tH)
                button:SetWide(tW)
	...
	if (!self.bInitialPopulate) then
		...
		if (#self.attributesPanel:GetChildren() > 1) then
			self.progress:AddSegment("@attributes")
		end
