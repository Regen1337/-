...
function PANEL:Paint(width, height)
    ...
    // if it's being dragged, don't draw the rect
...
function PANEL:PaintDragPreview(width, height, mouseX, mouseY, itemPanel)
	...
	if (item) then
		...
		if (IsValid(hoveredPanel) and hoveredPanel != itemPanel and hoveredPanel.GetItemTable) then
			...
			if (hoveredItem) then
				...
				if (info and info.OnCanRun and info.OnCanRun(hoveredItem, {item.id}) != false) then
					surface.SetDrawColor(ColorAlpha(derma.GetColor("Info", self, Color(200, 0, 0)), 50))
		...
		::finish::
		local previewColor = ColorAlpha(derma.GetColor(bEmpty and "Success" or "Error", self, Color(200, 0, 0)), 50)
