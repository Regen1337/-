...
-- function that gets a color that goes well with another color
local function GetContrastColor(color)
	local h, s, v = ColorToHSV(color)

	-- make a darker color
	v = math.Clamp(v - 0.2, 0, 1)
	s = math.Clamp(s + 0.2, 0, 1)
	h = h -500

	return HSVToColor(h, s, v)
end

local skin_texture = Material("sprites/physbeama" .. ".vmt")
local mat = Material("sprites/physcannon_bluelight2")
local rotationSpeed = 20 -- Adjust the speed as desired
function SKIN:DrawCoolHelixCurved(x, y, radius, segments, barHeight, fraction)
    radius = radius or math.min(ScreenScale(72), 128)
    segments = segments or 250
    barHeight = barHeight or 248
    fraction = fraction or 0.15

    local gradientStart = ix.config.Get("color") -- Starting color of the gradient
    local gradientEnd = GetContrastColor(gradientStart) -- Gradient color opposing the start color

    for i = 1, math.ceil(segments) do
        local angle = math.rad((i / segments) * 360)
        local barX = x + math.sin(angle + (fraction * math.pi)) * radius
        local barY = y + math.cos(angle + (fraction * math.pi)) * radius 
        
		local barOffset = math.sin( (i * 0.01))
        local barWidth = math.abs(barOffset) + 5
        local barColor = LerpColor(math.abs(barOffset), gradientEnd, gradientStart)

		local rotationAngle = math.rad(SysTime() * rotationSpeed)
        local rotatedBarX = (barX - x) * math.cos(rotationAngle) - (barY - y) * math.sin(rotationAngle) + x
        local rotatedBarY = (barX - x) * math.sin(rotationAngle) + (barY - y) * math.cos(rotationAngle) + y

		surface.SetMaterial(skin_texture)
		surface.SetDrawColor(barColor)
		surface.DrawTexturedRectRotated(rotatedBarX, rotatedBarY, barWidth, barOffset * (barHeight * fraction), math.deg(rotationAngle))
	end
end
...
--[==[
	...
	local mat = Material("sprites/physcannon_bluelight2")
	function SKIN:DrawCoolHelixCurved(x, y, radius, segments, barHeight, fraction)
		radius = radius or math.min(ScreenScale(72), 128) * 6
		segments = segments or 100
		barHeight = barHeight or 128
		fraction = fraction or 50

		local gradientStart = ix.config.Get("color")
		local gradientEnd = GetContrastColor(gradientStart) 
		local rotationSpeed = 15

		for i = 1, math.ceil(segments) do
			local angle = math.rad((i / segments) * 360)
			local barX = x + math.cos(angle + (fraction * math.pi * 2)) * radius
			local barY = y + math.cos(angle + (fraction * math.pi * 2)) * radius
			local barOffset = math.sin(SysTime() + i * 2)
			local barWidth = 4
			local barColor = LerpColor(math.abs(barOffset), gradientEnd, gradientStart)
			...
			local rotationAngle = math.rad(SysTime() * rotationSpeed)
			local rotatedBarX = (barX - x) * math.cos(rotationAngle) - (barY - y) * math.sin(rotationAngle) + x
			local rotatedBarY = (barOffset - x) * math.sin(rotationAngle) + (barY - y) * math.cos(rotationAngle) + y

			surface.SetMaterial(skin_texture)
			surface.SetDrawColor(barColor)
			surface.DrawTexturedRectRotated(rotatedBarX, rotatedBarY, barWidth, barOffset * (barHeight * fraction), math.deg(angle))

			--surface.SetMaterial(skin_texture)
			--surface.SetDrawColor(barColor)
			--surface.DrawTexturedRectRotated(barX, barY, barWidth, barOffset * (barHeight * fraction), math.deg(angle))
		end
	end
]==]
...
function SKIN:PaintFrame(panel)
	...
	if (panel:GetTitle() != "" or panel.btnClose:IsVisible()) then
		...
		surface.DrawTexturedRect(0, 0, panel:GetWide(), 24)
...
function SKIN:DrawImportantBackground(x, y, width, height, color)
	...
	surface.SetTexture(gradient)
	surface.SetDrawColor(color)
	surface.DrawOutlinedRect(x+1, 0+1, width-1, height-1)
	surface.DrawTexturedRect(x+1, y+1, width-1, height-1)
end
...
function SKIN:PaintSegmentedProgress(panel, width, height)
	...
	surface.SetDrawColor(barColor)
	surface.SetTexture(gradient)
	surface.DrawTexturedRect(0, height - height * 0.16, panel:GetFraction() * width, height * 0.15)
