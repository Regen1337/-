...
function PANEL:SetItem(itemTable)
	...
	self.icon:SetItemTable(itemTable)
	...
	local color = itemTable.GetColor and itemTable:GetColor()
	self.icon.ExtraPaint = function(this, panelX, panelY)
...
function PANEL:Init()
    ...
    self.categories:DockMargin(0, 30, 0, 0)
	self.categories.Paint = function(this, w, h)
		surface.SetDrawColor(0, 0, 0, 150)
		surface.DrawRect(0, 0, w, h)
	end
	self.categories:DockPadding(5, 0, 5, 5)
	self.categoryPanels = {}
	...
	self.search:SetFont("RMediumFont")
	...
	for category, realName in SortedPairs(self.categoryPanels) do
		local button = self.categories:Add("RMenuSelectionButton")
		button:SetTall(36)
...
function PANEL:Init()
	...
	self.buy = self:Add("RMenuSelectionButton")
	self.buy:Dock(BOTTOM)
	self.buy:SetText(L"purchase")
    local tW, tH = self.buy:GetTextSize()
    tW = tW
    self.buy:SetWide(tW)
	...
	self.current:SetFont("RSmallFont")
	...
	self.finalGlow:SetFont("RSmallFont")
...
function PANEL:SetCart(items)
	...
	for k, v in SortedPairs(items) do
		...
		if (itemTable) then
			...
			slot.icon:SetZPos(9999)
			slot.icon:SetPos(2, 2)
			slot.icon:SetSize(32, 32)
			slot.icon:SetModel(itemTable:GetModel(), itemTable:GetSkin())
			slot.icon:SetTooltip()
			
			local color = itemTable.GetColor and itemTable:GetColor()
			slot.icon.ExtraPaint = function(this, panelX, panelY)
				local exIcon = ikon:GetIcon(itemTable.uniqueID)
				if (exIcon) then
					surface.SetMaterial(exIcon)
					surface.SetDrawColor(color and color or color_white)
			...
			slot.quantity:SetFont("RChatFont")
