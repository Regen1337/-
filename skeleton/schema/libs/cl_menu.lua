...
function PANEL:Init()
	...
	-- main button panel
	self.buttons = self:Add("RHorizontalScroller")
	self.buttons:Dock(TOP)
	self.buttons:SetPaintedManually(true)
	self.buttons.Paint = function() 
		surface.SetDrawColor(0, 0, 0, self.currentAlpha)
		surface.SetTexture(gradient)
		surface.DrawTexturedRect(0, 0, self:GetWide(), self.buttons:GetTall() * 2)
	end


	local close = vgui.Create("RMenuSelectionButton")
	close:SetText(L"return")
	close:Dock(LEFT)
	close:SizeToContents()

	local tW, tH = self:GetTextSize("ixMenuButtonFont", close:GetText())

	close:SetBackgroundColor(ix.config.Get("color"))

	close:SizeToContents()
	self.buttons:AddPanel(close)
	close.DoClick = function()
		self:Remove()
	end

	local characters = vgui.Create("RMenuSelectionButton")
	characters:SetText(L"characters")
	...
	-- tabs
	self.tabs = vgui.Create("Panel")
	self.tabs.buttons = {}
	self.tabs:SetWide(self:GetWide() / 15)
	self.buttons:AddPanel(self.tabs)
	self:PopulateTabs()
...
function PANEL:GetTextSize(font, text)
...
function PANEL:SetWideFromText(panel)
	if (!IsValid(panel)) then return end
	local font = panel:GetFont(); local text = panel:GetText();
	local w, h = panel:GetTextSize(font, text)
	panel:SetWide(w)
...
function PANEL:GetStandardSubpanelSize()
	...
	return sW - (sW * 0.1), sH - self.buttons:GetTall() - (sH * 0.1) 
end
...
function PANEL:SetupTab(name, info, sectionParent)
	...
	if (sectionParent) then
		...
		subpanel:SetPos(0, self.buttons:GetTall() + 32)
		subpanel:SetSize(self:GetStandardSubpanelSize())
	else
		subpanel:SetPos(0, self.buttons:GetTall() + 32)
		subpanel:SetSize(self:GetStandardSubpanelSize())
	...
	local button = vgui.Create("RMenuSelectionButton")
