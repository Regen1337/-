...
-- find empty slots for a table of items, making sure to not occupy slots that are already occupied
-- this is used for when you want to add multiple items to an inventory at once
-- @realm shared
-- @internal
-- @table items The table of items to find empty slots for (uses the item's width and height)
-- @bool onlyMain Whether or not to search any bags connected to this `Inventory`
-- @treturn bool Whether or not all of the items can fit in the inventory

-- use a cache to blacklist slots that get occupied by items and loop through all slots
-- use the blacklist slots to make sure items do not occupy the same slots on the next iteration
function META:FindEmptySlots(items, onlyMain)
	local slots = {}
	local blacklist = {}
